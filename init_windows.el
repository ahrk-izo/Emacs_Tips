...
;; (add-to-list 'package-archives '("marmalade" . "http://marmalade-repo.org/packages/")) ;;なぜかエラーになり重くなるのでやめた方がいい
(package-initialize)
...
;; auto-installの設定(エラーになるからだめだ！！必要なものは手動でelispに保存するか・・・)
...
(setq default-frame-alist
    ...
    '(top . 0)                         ;フレーム左上位置 Y座標
    '(left . 300)                        ;フレーム左上位置 X座標
    '(width . 195)                     ;フレーム幅(文字数)
    '(height . 68)                     ;フレーム高さ(文字数)
...
;; (prefer-coding-system 'utf-8)      ;eshell等もutf-8になる。windowsではそれはよろしくない
...
(define-key global-map (kbd "C-c o") 'occur)                     ;便利な検索
...
(define-key global-map (kbd "C-c C-.") 'delete-horizontal-space) ;周りの空白を削除
...
(define-key global-map (kbd "C-c C-a") 'sort-lines)              ;ソート(これを登録したので、逆順はC-u C-c C-aでOK)
...
(define-key global-map (kbd "C-c C-t") 'anything-for-files)      ;anything-for-files(diredではC-tが他に割り当てられている)
...
(define-key global-map (kbd "M-s") 'paredit-split-sexp)   ;括弧を分割して増やす
...
(require 'wgrep nil t)
;; *grep*バッファで、r:編集、C-x C-s:編集完了(この時点ではファイルは編集中)、C-c C-k:編集破棄
...
;; 現在の日付の挿入(数字のみ)
(defun insert-current-date-nun-only()
  (interactive)
  (insert (format-time-string "%Y%m%d" (current-time))))
(define-key global-map (kbd "C-c C-]") `insert-current-date-nun-only)
...
;; M-;2回で#=>, その状態でM-pを実行すると出力される。ただし、パスに日本語があると、エラーになる。
